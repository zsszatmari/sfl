...
- (void)refresh
{
    ...
    NSIndexSet *selected = [self.imageBrowserView selectionIndexes];
    if ([selected count] > 0) {
        selectedItem = [selected firstIndex];
        //NSLog(@"selected: %ld", (unsigned long)selectedItem);
    }
    
    if (visibleGroupings.size() > 0) {
        [self.imageBrowserView setAnimates:NO];
    }
    ...
    if (selectedItem != NSNotFound) {
        selfInducedSelection = YES;
        [self.imageBrowserView setSelectionIndexes:[NSIndexSet indexSetWithIndex:selectedItem] byExtendingSelection:NO];
        selfInducedSelection = NO;
    }*/
}
...
/*
    ...
    if (!reloading) {
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            ...
            SongGrouping visibleItem;
            NSIndexSet *visible = [self.imageBrowserView visibleItemIndexes];
            if ([visible count] > 0 && [visible firstIndex] > 0) {
                NSUInteger index = ([visible firstIndex] + [visible lastIndex])/2;
                auto size = visibleGroupings.size();
                if (index >= size) {
                    if (size > 0) {
                        index = size-1;
                    }
                ...
                if (index < size) {
                    visibleItem = visibleGroupings.at(index);
                }
            }
            SongGrouping selectedItem;
            NSIndexSet *selected = [self.imageBrowserView selectionIndexes];
            if ([selected count] > 0) {
                NSUInteger index = [selected firstIndex];
                if (index < visibleGroupings.size()) {
                    selectedItem = visibleGroupings.at(index);
                }
            }
            ...
            if (visibleItem) {
                ...
                auto it = find(visibleGroupings.begin(), visibleGroupings.end(), visibleItem);
                
                if (it != visibleGroupings.end()) {
                    auto visibleIndex = std::distance(visibleGroupings.begin(), it);
                    [self.imageBrowserView scrollIndexToVisible:visibleIndex];
                }
                
                if (selectedItem) {
                    auto itSelected = find(visibleGroupings.begin(), visibleGroupings.end(), selectedItem);
                    if (itSelected != visibleGroupings.end()) {
                        
                        auto index = std::distance(visibleGroupings.begin(), itSelected);
                        selfInducedSelection = YES;
                        [self.imageBrowserView setSelectionIndexes:[NSIndexSet indexSetWithIndex:index] byExtendingSelection:NO];
...
- (void)selectItemAtIndexByUser:(NSUInteger)index
{
    if (index < visibleGroupings.size()) {
        ...
        _browserPlaylist->selectGrouping(selected);
