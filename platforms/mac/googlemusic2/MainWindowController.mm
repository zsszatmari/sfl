...
@interface MainWindowController () {
    ...
    vector<WeakSongEntry> contextMenuSongs;
...
- (void)setConstraintsForNowPlayingShown:(BOOL)shown
{
    ...
    auto spacing = IApp::instance()->themeManager()->style().get("playlistpanel nowplayingtitle").margin();
...
- (void)updatePlayImage
{
    ...
    auto set = [](CGFloat diff){
        NSEnumerator *enumerator = [views keyEnumerator];
        NSView *view;
         
        while ((view = [enumerator nextObject])) {
            CGRect frame = view.frame;
            frame.origin.y = -([[views objectForKey:view] floatValue] + diff) + view.superview.frame.size.height - view.frame.size.height;
            view.frame = frame;
        }
    };
    
    if (gradient) {
       /* NSImage *image = playing ? [[NSImage imageNamed:@"pause"] imageTintedWithColorKeepSize:[NSColor blackColor]] : [[NSImage imageNamed:@"play"] imageTintedWithColorKeepSize:[NSColor blackColor]];
        [playButton setImage:image];
        [nextButton setImage:[[NSImage imageNamed:@"next"] imageTintedWithColorKeepSize:[NSColor blackColor]]];
        [prevButton setImage:[[NSImage imageNamed:@"prev"] imageTintedWithColorKeepSize:[NSColor blackColor]]];*/
        
        NSImage *image = playing ? [NSImage imageNamed:@"pauseblack"] : [NSImage imageNamed:@"playblack"];
        [playButton setImage:image];
        [nextButton setImage:[NSImage imageNamed:@"nextblack"]];
        [prevButton setImage:[NSImage imageNamed:@"prevblack"]];
        [[nextButton cell] setXOffset:1];
        [[prevButton cell] setXOffset:-1];
        
        //set(-5);
        
    } else {
        NSImage *image = playing ? [NSImage imageNamed:@"pause"] : [NSImage imageNamed:@"play"];
        [playButton setImage:image];
        [nextButton setImage:[NSImage imageNamed:@"next"]];
        [prevButton setImage:[NSImage imageNamed:@"prev"]];
        [[nextButton cell] setXOffset:0];
        [[prevButton cell] setXOffset:0];
...
- (void)adjustCollection
{
    ...
    if (playlist && playlist->groupingsEnabled()) {
        ...
        if (!playlist->selectedGroupingConnector().value()) {
...
- (void)showBigText
{
    ...
    for (int i = 0 ; i < len ; i++) {
        ...
        [attrString setAttributes:@{NSParagraphStyleAttributeName:style, NSForegroundColorAttributeName:[baseColor colorWithAlphaComponent:(1-ratio)]} range:NSMakeRange(i,1)];
    }
...
- (void)showCount:(size_t)count
{
    ...
    if (shadow) {
        ...
        [songCountLabel setAttributedStringValue:[[NSAttributedString alloc] initWithString:countString attributes:@{NSShadowAttributeName:shadow,
                                                                                                                     NSParagraphStyleAttributeName:paragraphStyle}]];
    } else {
...
- (void)jumpToSong
{
    ...
    playlistCurrentlyPlaying->selectGrouping(player->groupingCurrentlyPlaying());
    dispatch_async(dispatch_get_main_queue(),^{
...
- (void)jumpToSongForSong:(SongEntry)song animate:(BOOL)animate userInitiated:(BOOL)userInitiated
{
    ...
    if (songStillValid || (!userInitiated && CGRectGetMinY([musicListViewScroll documentVisibleRect]) != 0)) {
        ...
        scrollOrigin.y = (int)(scrollOrigin.y + (rowRect.size.height - viewRect.size.height) / 2);
...
- (void)resizeTopViews
{
    ...
    frame.origin.x = self.prevButton.frame.origin.x + 34;
    self.playButton.frame = frame;
    frame = self.nextButton.frame;
    frame.origin.x = self.playButton.frame.origin.x + 38;
    ...
    if (field == nil) {
        if ([tableColumn.identifier isEqualToString:@"source"]) {
            ...
            if ([tableColumn.identifier isEqualToString:@"rating"]) {
                ...
                NSControl *control = [[NSTextField alloc] init];
                control.cell = [self tableView:tableView dataCellForTableColumn:tableColumn row:row];
...
static WeakSongEntry lastClickEntry;
...
{
    SongEntry entry = lastClickEntry.lock();
    if (entry) {
...
- (void)contextMenuIntent:(id)sender
{
    if ([sender isKindOfClass:[MenuItemWithIntent class]]) {
        ...
        if (intent) {
            ...
            for (auto &weakSong : contextMenuSongs) {
                auto song = weakSong.lock();
                if (song) {
...
- (NSMenu *)tableView:(NSTableView *)tableView menuForRows:(NSIndexSet *)rows column:(NSUInteger)column
{
    ...
    contextMenuSongs.clear();
    transform(songs.begin(), songs.end(), back_inserter(contextMenuSongs), [](const SongEntry &entry){
        return WeakSongEntry(entry);
    });
