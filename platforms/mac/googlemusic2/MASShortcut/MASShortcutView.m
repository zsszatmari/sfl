...
- (void)setShortcutValue:(MASShortcut *)aShortcutValue
{
    if (_shortcutValue != aShortcutValue) {
        [_shortcutValue release];
        _shortcutValue = aShortcutValue;
        [_shortcutValue retain];
    }
    [self resetToolTips];
    [self setNeedsDisplay:YES];
...
- (void)drawInRect:(CGRect)frame withTitle:(NSString *)title alignment:(NSTextAlignment)alignment state:(NSInteger)state
{
    ...
    switch (_appearance) {
        case MASShortcutViewAppearanceDefault: {
            [_shortcutCell drawWithFrame:NSRectFromCGRect(frame) inView:self];
            break;
        }
        case MASShortcutViewAppearanceTexturedRect: {
            [_shortcutCell drawWithFrame:NSRectFromCGRect(CGRectOffset(frame, 0.0, 1.0)) inView:self];
            break;
        }
        case MASShortcutViewAppearanceRounded: {
            [_shortcutCell drawWithFrame:NSRectFromCGRect(CGRectOffset(frame, 0.0, 1.0)) inView:self];
...
- (void)drawRect:(NSRect)dirtyRect
{
    if (self.shortcutValue) {
        [self drawInRect:NSRectToCGRect(self.bounds) withTitle:MASShortcutChar(self.recording ? kMASShortcutGlyphEscape : kMASShortcutGlyphDeleteLeft)
        ...
        if (self.recording)
        {
            [self drawInRect:NSRectToCGRect(self.bounds) withTitle:MASShortcutChar(kMASShortcutGlyphEscape) alignment:NSRightTextAlignment state:NSOffState];
        ...
        {
            [self drawInRect:NSRectToCGRect(self.bounds) withTitle:NSLocalizedString(@"Record Shortcut", @"Empty shortcut button in normal state")
...
- (void)activateEventMonitoring:(BOOL)shouldActivate
{
    ...
    if (shouldActivate) {
        ...
        eventMonitor = [NSEvent addLocalMonitorForEventsMatchingMask:eventMask handler:^(NSEvent *event) {
            ...
            /*if (shortcut.modifierFlags > 0 && [[shortcut description] length] > 1) {
