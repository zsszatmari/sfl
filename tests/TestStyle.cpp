...
{
	Style style(kStyle);
	auto rules = style.get("controls play pushed");
	auto imageName = rules.imageName();
	EXPECT_EQ("play_pushed", imageName);

	auto paintable = rules.paintable();
	auto casted = dynamic_cast<const NamedImage *>(paintable.get());
	ASSERT_TRUE(casted);
	EXPECT_EQ("play_pushed", casted->imageName());
}
