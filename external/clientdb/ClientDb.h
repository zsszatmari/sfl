...
namespace ClientDb
{
	...
	public:
		...
		template<class T>
		shared_ptr<View<T>> view(const std::string &entity, const Predicate &predicate, const SortDescriptor &orderBy, const std::function<void(const std::function<void()> &)> &viewQueue, const std::function<void(const shared_ptr<View<T>> &,IView::Event,size_t,size_t)> &callback)
		{
			auto self = shared_from_this();

			shared_ptr<View<T>> ret(new View<T>(entity, viewQueue, callback, self, orderBy, predicate));
	...
	private:
		...
		void enumerateViews(const std::function<void(const shared_ptr<IView> &)> &f);
		...
		std::vector<weak_ptr<IView>> _views;
