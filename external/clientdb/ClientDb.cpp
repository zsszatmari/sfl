...
#define method Db::
	...
	void method upsert(const std::string &entity, const Json::Value &object, const std::function<void(const Json::Value &, Json::Value &)> &oldObjectHandler)
	{
		...
		_executor.addTask([self,this,entity,object,oldObjectHandler]() {
			...
			enumerateViews([&](const shared_ptr<View> &view){
				if (oldObject.isNull()) {
	...
	void method enumerateViews(const std::function<void(const shared_ptr<View> &)> &f)
	{
		// must be called from db thread
		_views.erase(remove_if(_views.begin(), _views.end(), [&](const weak_ptr<View> &w){
			auto view = w.lock();
			if (view) {
				f(view);
				return false;
	...
	shared_ptr<View> method view(const std::string &entity, const Predicate &predicate, const SortDescriptor &orderBy, const std::function<void(const std::function<void()> &)> &viewQueue, const std::function<void(const shared_ptr<View> &,View::Event,size_t,size_t)> &callback)
	{
		auto self = shared_from_this();

		shared_ptr<View> ret(new View(entity, viewQueue, callback, self, orderBy, predicate));
		_executor.addTask([self,ret]{
			self->_views.push_back(ret);
		});
