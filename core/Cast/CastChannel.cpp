...
#define method CastChannel::
    ...
    void method connect(const string &host, uint16_t port, const function<void(int)> &handler)
    {
        ...
        _socket.lowest_layer().async_connect(destination, [self,handler](boost::system::error_code ec){
            if (ec) {
                handler(ec.value());
                return;
            }
            self->_localHost = self->_socket.lowest_layer().local_endpoint().address().to_string();
            Base::Logger::stream() << "connected to chromecast. it can reach us at " << self->_localHost;
