...
#define method CastPlaybackWorker::
    ...
    {
        _playing = false;
        // let it be 1 by default, because 
        _volume = 1.0f;
    }
    ...
    void method probe()
    {
        ...
        _probeTimer.async_wait([self](const boost::system::error_code e){
            ...
            self->_devices.set([&](vector<AudioDevice> &v){
                auto activeDeviceNames = map([](const shared_ptr<Cast::CastDevice> &d){return d->name();}, self->_activeDevices);
                v.erase(remove_if(v.begin(), v.end(), [&](const AudioDevice &device){
                    if (device.lastSeenSecondsBefore() > 170 && !elem(device.name(),activeDeviceNames)) {
                        change = true;
                        return true;
                    } else {
                        return false;
...
*/
    ...
    void method connect(const shared_ptr<CastDevice> &device)
    {
        ...
        if (!_streamer) {
            ...
            new Base::Timer([generator,streamer,&playing,&io,this](Timer *timer){
                ...
                do {
                    ...
                    #if 1
                    io.post([generator,streamer,isPlaying,this]{
