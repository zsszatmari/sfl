...
#define method GooglePlayConceiver::
    ...
    static vector<Json::Value> entriesFromIds(const vector<string> &ids)
    {
    ...
    const vector<Json::Value> method songs(const string &playlistId) const
    {
        vector<Json::Value> ret;
        ...
        switch(_hint) {
            ...
            case Hint::addToLibrary: {
                ...
                for (auto itPair = songs.begin() ; itPair != songs.end() ; ++itPair) {
                    ...
                    Json::Value entry;
                    entry["playlist"] = playlistId;
                    entry["id"] = allAccessId;
                    entry["entryId"] = libraryId;
                    entry["matchedKey"] = allAccessId;
        ...
        for (auto itSongJson = songs.begin() ; itSongJson != songs.end() ; ++itSongJson) {
            ...
            try {
                ...
                //std::cout << "song: " << writer.write(songJson) << std::endl;
    #endif
                ...
                song["title"] = songJson[keysToIndexes["title"]].asString();
                ...
                if (providedArt.empty() || providedArt == "0") {
                    ...
                    if (providedArt.compare(0, kHttp.length(), kHttp) == 0 || 
                        ...
                        song["albumArtUrl"] = providedArt;
                    } else {
                        song["albumArtUrl"] = kHttp + providedArt;
                        
                    }                
    #ifdef DEBUG
                    string debUrl = song["albumArtUrl"].asString();
    #endif
                    
                }
                song["artist"] = songJson[keysToIndexes["artist"]].asString();
                song["album"] = songJson[keysToIndexes["album"]].asString();
    		    // 11 genre?
                auto matchedKey = songJson[28].asString(); // 27 is wrong
                if (matchedKey != "0") {
                	song["matchedKey"] = matchedKey;
                }
                song["albumArtist"] = songJson[keysToIndexes["albumArtist"]].asString();
                song["albumId"] = songJson[32].asString();
                song["artistId"] = songJson[33].asString();
                song["durationMillis"] = songJson[13].asUInt64();
                song["track"] = songJson[keysToIndexes["track"]].asUInt64();
                song["disc"] = songJson[keysToIndexes["disc"]].asUInt64();
                song["year"] = songJson[keysToIndexes["year"]].asUInt64();

                // songJson[29] means access level, statistics: [(1,2),(2,11669),(6,3008),(7,691)]
                // see 'googledata' project
                // 1: bought
                // 2: uploaded by me (and downloadable)
                // 6: uploaded by me, and matched (downloadable)
                // 7: all access (no download possible officially)
                //song->setUintForKey("googleAccess", songJson[29].asUInt64());
                ...
                if (playCount > 0 && songJson.size() >= 49) {
                    song["lastPlayed"] = songJson[48].asUInt64();
                }
                song["creationDate"] = songJson[24].asUInt64();
                ...
                ret.push_back(std::move(song));
            } catch (...) {
                Json::FastWriter writer;
                std::cout << "error parsing: " << writer.write(songJsonPure) << std::endl;
                throw;
            }
        }
        return std::move(ret);
    ...
    static const vector<Json::Value> playlistsFrom(const Json::Value &playlists, const shared_ptr<GooglePlaySession> &session)
    {
        ...
        for (auto itPlaylist = playlists.begin() ; itPlaylist != playlists.end() ; ++itPlaylist) {
            ...
            */
            ...
            // this was some kind of mechanism to not show radios associated with deleted songs
            /*
            ...
            if (source[0].asInt() == 0) {
                ...
                if ((bound == libraryIds.end() || *bound != songId)) {
                    continue;
                }
            }*/
            ...
            radio["dynamic"] = true;
            radio["name"] = playlist[1].asString();
            radio["playlistId"] = playlist[0].asString();
            ret.push_back(std::move(radio));
        }
        return std::move(ret);
    }
    
    const vector<Json::Value> method playlists() const
    {
        vector<Json::Value> ret;
