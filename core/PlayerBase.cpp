...
#define snprintf _snprintf
    ...
    void method updateNextSong()
    {
        ...
        if (_cachedSong) {
            ...
            auto session = retainedSong->session();
            if (session) {
                session->playbackData(*retainedSong, [retainedSong,self,this](const shared_ptr<IPlaybackData> &data){
                    self->cacheSong(retainedSong, data);
                });
            }
        }
    }
    ...
    void method play(const shared_ptr<ISong> &aSongToPlay)
    {
        ...
        Io::get().dispatch([songToPlay, pThis]{
