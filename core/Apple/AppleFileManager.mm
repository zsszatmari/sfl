...
#define method AppleFileManager::
    ...
    vector<string> method listFiles(const string &prefix)
    {
        ...
        @autoreleasepool {
            ...
            NSFileManager *fm = [NSFileManager defaultManager];
            NSArray *enumerator = [[fm contentsOfDirectoryAtURL:[NSURL fileURLWithPath:convertString(dir)] includingPropertiesForKeys:@[NSURLContentAccessDateKey, NSURLFileSizeKey] options:0 error:nil] mutableCopy];
            ...
            for (NSURL *url in enumerator) {
                ...
                if ([size intValue] == 0) {
                    // do not delete this! it doesn't hurt to leave some 0 byte files, and 
                    // it may be that we are downloading something at the present moment
