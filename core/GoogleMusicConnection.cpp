...
#define method GoogleMusicConnection::
    ...
    void method startDownload(const string &name, const Json::Value &param, const string &m, const function<void(const string &)> &callback, int chunkSize)
    {
        ...
        AsyncHttp::Request request(url, headers, m, m == "POST" ? body : "");
        auto gotResponse = [callback](const AsyncHttp::Response &response){
            if (response.ok()) {
                callback(response.bodyAsString());
            } else {
                callback("");
            }
        };
        if (chunkSize == 0) {
            AsyncHttp::download(Io::get(), request, gotResponse);
        } else {
            AsyncHttp::download(Io::get(), request, chunkSize, gotResponse);
        }
    }
    
    void method callPure(const std::string &name, const function<void(const std::string &)> &callback)    
    {
        map<string, string> headers = buildHeaders();
        ...
        AsyncHttp::Request request("https://play.google.com/music/" + name, headers, "GET", "");
        AsyncHttp::download(Io::get(), request, [callback](const AsyncHttp::Response &response){
            callback(response.ok() ? response.bodyAsString() : "");
        });
    }
    
    void method streamingCall(const string &name, const Json::Value &param, const function<void(const Json::Value &)> &gotChunk)
    {
        shared_ptr<JsonStreamer> streamer(new JsonStreamer());
        startDownload(name, param, "GET", [streamer,gotChunk](const string &str){
            streamer->gotData(str, gotChunk);
        }, JsonStreamer::ChunkSize);
    }
    ...
    {
        startDownload(name, param, "POST", [name,callback](const string &aStr){
            if (!aStr.empty()) {
