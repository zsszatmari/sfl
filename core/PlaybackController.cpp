...
#define method PlaybackController::
    ...
    void method updatePlayCount(const shared_ptr<ISong> &song)
    {
        ...
        song->updateInDb({"playCount","lastPlayed"});
    }    
    ...
    bool method playSong(shared_ptr<ISong> song)
    {
        ...
        if (song && equals(_songPlayed, song)) {
            ...
            if (!shouldSkip) {
                auto callback = [this,song](const shared_ptr<IPlaybackData> &data){
                    assert(Io::isCurrent());
...
#pragma message("TODO: find out what return false really meant here... I am sure it was important, and it's something related to not continue playback after some fails")
                    }
                };
                auto session = song->session();
                if (!session) {
                    callback(shared_ptr<IPlaybackData>());
                } else {
                    session->playbackData(*song, callback);
                }
            }
        }
        return true;
