...
namespace Gear
{
	...
	public:
		...
		virtual std::string registerView(const std::string &entity, const ClientDb::Predicate &predicate, const ClientDb::SortDescriptor &orderBy, const std::function<void(const std::function<void()> &)> &viewQueue, const std::function<void(ClientDb::View::Event,size_t,size_t)> &callback) = 0;
		virtual void unregisterView(const std::string &handle) = 0;

		virtual Json::Value viewAt(const std::string &handle, size_t index) const = 0;
