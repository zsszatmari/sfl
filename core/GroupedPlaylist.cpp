...
#define method GroupedPlaylist::
    ...
    method GroupedPlaylist(const shared_ptr<SortedSongArray> &base, const string &name, const vector<string> &groupBy) :
        IPlaylist(weak_ptr<ISession>()),
        _name(name),
        _playlistId(createId(groupBy)),
        _array(base),
        _filteredArray(PredicateSongArray::create(base,undefined())),
        _groupBy(groupBy),
        _calculateId(0)
    {
    }
    ...
    shared_ptr<GroupedPlaylist> method create(const shared_ptr<SortedSongArray> &base, const string &name, const vector<string> &groupBy)
    {
        shared_ptr<GroupedPlaylist> ret(new GroupedPlaylist(base, name, groupBy));
    ...
    void method selectGrouping(const SongGrouping &grouping)
    {
#ifdef DEBUG
        std::cout << "select grouping: " << grouping.title() << std::endl;
#endif
        
        IPlaylist::selectGrouping(grouping);
        
        if (grouping) {
            _filteredArray->setInherentPredicate(MEMORY_NS::static_pointer_cast<SortedSongArray>(_array)->inherentPredicate());
            _filteredArray->setSecondaryFilterPredicate(grouping.predicate());
        } else {
            _filteredArray->setSecondaryFilterPredicate(undefined());
        }
    }
    ...
    vector<SongGrouping> method groupSongs(const ClientDb::Predicate &aPredicate, const vector<string> &groupBy)
    {
        ...
        for (auto &item : data) {
            ...
            SongGrouping processing(title, subtitle, groupKey, title, multiImage, albumInfo);
