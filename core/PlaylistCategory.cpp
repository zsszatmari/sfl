...
#define method PlaylistCategory::
    ...
    method PlaylistCategory(const string &title, int tag, bool defaultSelection, const vector<shared_ptr<IPlaylist>> &playlists) :
        _title(title),
        ...
        _playlists(playlists)
    {
        switch(tag) {
            ...
            case kPlaylistsTag:
                ...
                break;
            case kSubscriptionsTag:
                _icon = shared_ptr<IPaintable>(new NamedImage("category-subscriptions"));
                break;
        }
    }
