...
namespace Gear
{
	...
	public:
		...
		void upsert(const SongEntry &song);
		void upsert(const shared_ptr<SongEntrySong> &song);
		void freeUpMemory();

		virtual std::string registerView(const std::string &entity, const ClientDb::Predicate &predicate, const ClientDb::SortDescriptor &orderBy, const std::function<void(const std::function<void()> &)> &viewQueue, const std::function<void(ClientDb::View::Event,size_t,size_t)> &callback);
		virtual void unregisterView(const std::string &handle);

		virtual Json::Value viewAt(const std::string &handle, size_t index) const;
	...
	private:
		...
		std::map<std::string,shared_ptr<ClientDb::View>> _views;
	};
}
