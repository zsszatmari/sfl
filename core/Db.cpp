...
#define method Db::
	...
	static std::vector<ClientDb::Index> indexes()
	{
...

		{
			...
			// primary index
			// (source,playlist,entryId) is needed for mass deletion see Db::remove()
...

		{
			...
			cols.push_back("artistId");
			cols.push_back("matchedKey");
			cols.push_back("savedForOffline");
		...
		{
			vector<std::string> cols = {"source","playlistId","name"};
		...
		_db(ClientDb::Db::openSync(path, indexes(), queue(), shared_ptr<IBackend>(new SqliteBackend()), &StringCompare::compareWithLength))
	{
		...
		bridge->installSubscribe("view", [this](const Json::Value &arg, const function<void(const Json::Value &)> &callback){
			...
			auto view = _db->view(entity, predicate, orderBy, viewQueue, [callback](const shared_ptr<View> &,ClientDb::View::Event event,size_t offset,size_t size){
	...
	void method upsert(const SongEntry &song)
	{
		_db->upsert("Song", song.values());
	}

	void method upsert(const shared_ptr<SongEntrySong> &song)
	{
		_db->upsert("Song", song->_values);
	}
	...
	std::string method registerView(const std::string &entity, const ClientDb::Predicate &predicate, const ClientDb::SortDescriptor &orderBy, const std::function<void(const std::function<void()> &)> &viewQueue, const std::function<void(ClientDb::View::Event,size_t,size_t)> &callback)
	{
		auto view = _db->view(entity, predicate, orderBy, viewQueue, [callback](const shared_ptr<View> &,ClientDb::View::Event event,size_t offset,size_t size){
			callback(event, offset, size);
		});
	...
	Json::Value method viewAt(const std::string &handle, size_t index) const
	{
